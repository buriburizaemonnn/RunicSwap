...
async fn transform_response(
  ...
  *response.headers_mut() = remove_hop_headers(response.headers());
  ...
  println!("Collected response body size: {}", bytes.len());
  println!("Response body content: {}", String::from_utf8_lossy(&bytes));
...
pub async fn call(
  ...
  println!(
    "Sending request to Bitcoin node with body: {}",
  ...
  let proxied_request = Request::builder()
    ...
    .header(
      "Authorization",
      format!("Basic {}", base64::encode("icp:test")),
    )
    .body(Full::from(body_bytes))
    .expect("failed to convert into proxy request");
