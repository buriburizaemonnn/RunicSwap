...
pub fn derive_public_key(ecdsa_pukey: EcdsaPublicKey, path: &[ByteBuf]) -> EcdsaPublicKey {
    let path = DerivationPath::new(
        path.iter()
            .map(|x| DerivationIndex(x.clone().into_vec()))
            .collect(),
    );
    let pk = PublicKey::deserialize_sec1(&ecdsa_pukey.public_key)
        .expect("failed to parse ECDSA public key");

    let chain_code: [u8; 32] = ecdsa_pukey
        .chain_code
        .clone()
        .try_into()
